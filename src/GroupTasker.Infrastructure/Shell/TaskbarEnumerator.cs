...
public sealed class TaskbarEnumerator : ITaskbarEnumerator
{
    ...
    public IReadOnlyList<DiscoveredApp> Enumerate()
    {
        ...
        // De-dup. Preference: real MSIX AUMI first (most launchable), then
        // shortcut-backed entries (they carry the correct launcher arguments),
        // then store mirrors, pins, and the most complete remaining entry.
        return results
            .GroupBy(a => DedupKey(a))
            .Select(g => g
                .OrderByDescending(a => a.Aumi is not null && a.Aumi.Contains('!'))
                ...
                .First())
            .OrderBy(a => SourceRank(a.Source))
            .ThenBy(a => a.DisplayName, StringComparer.OrdinalIgnoreCase)
            .ToList();
    }

    /// <summary>
    /// Visual order in the picker: games/launchers first (the most requested),
    /// then taskbar pins, store apps, and running processes.
    /// </summary>
    private static int SourceRank(DiscoveredAppSource source) => source switch
    {
        DiscoveredAppSource.GameLibrary => 0,
        DiscoveredAppSource.PinnedTaskbar => 1,
        DiscoveredAppSource.StoreApp => 2,
        _ => 3
    };
    ...
    {
        // Known launchers merge across sources: desktop .lnk, Start Menu .lnk,
        // AppsFolder entry, and taskbar pin all refer to the same app.
        if (GameLibraryEnumerator.IsLauncherName(a.DisplayName))
            return $"name:{NormalizeDedupName(a.DisplayName)}";

        // Launcher protocol URLs identify the game: merge a desktop .url, the
        // AppsFolder steam:// entry, and the Steam-library scan into one item.
        if (!string.IsNullOrEmpty(a.TargetUrl)) return $"url:{a.TargetUrl.ToLowerInvariant()}";

        if (!string.IsNullOrEmpty(a.LnkPath))
        {
            if (a.Source != DiscoveredAppSource.PinnedTaskbar)
            {
                if (!string.IsNullOrEmpty(a.ExecutablePath))
                {
                    if (!GameLibraryEnumerator.IsLauncherExe(a.ExecutablePath))
                    {
                        return $"exe:{a.ExecutablePath.ToLowerInvariant()}";
                    }
                    // Launcher-targeted game shortcuts (VALORANT → RiotClientServices.exe,
                    // Diablo → Battle.net Launcher.exe): the identity is the shortcut's
                    // name + arguments, so key by name. This also merges with an
                    // AppsFolder mirror of the same shortcut.
                    return $"name:{NormalizeDedupName(a.DisplayName)}";
                }
            }
        ...
        if (!string.IsNullOrEmpty(a.Aumi))
        {
            ...
            // "Microsoft.AutoGenerated.{GUID}" entries are AppsFolder mirrors of
            // plain Start Menu shortcuts — merge them by name with the
            // GameLibrary entry for the same shortcut.
            if (family.StartsWith("microsoft.autogenerated.", StringComparison.OrdinalIgnoreCase))
                return $"name:{NormalizeDedupName(a.DisplayName)}";
    ...
    /// <summary>
    /// Normalise a display name for dedup: trim, lower-case, and strip
    /// Windows' duplicate-shortcut suffixes so "Xbox (2)" merges with "Xbox".
    /// </summary>
