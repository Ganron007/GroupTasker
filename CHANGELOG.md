...
- **Desktop shortcuts are now discovered.** The picker scans the user + public
  desktop (including the physical desktop on OneDrive-redirected systems), so
  launchers and game shortcuts on the desktop appear in the list without
  browsing. Verified end-to-end against a real machine with 9 desktop gaming
  shortcuts — all appear.
- **Category quick-filter in the picker** — All / 🎮 Games / 📌 Taskbar /
  ⊞ Store / ▶ Running. Games and launchers now sort to the top of the list
  instead of being buried under ~300 store-app entries.
- **The + Add file picker opens on the Desktop**, where launchers and game
  shortcuts actually live.

### Fixed

- **Duplicate entries for the same game/launcher.** Desktop, Start Menu,
  AppsFolder, and Steam-library copies of the same item now merge into one:
  known launcher names merge across sources, launcher protocol URLs
  (`steam://rungameid/…`) merge across desktop `.url` / AppsFolder / library
  scans (Steam appids are read from `appmanifest_*.acf`), and AppsFolder
  mirrors (`Microsoft.AutoGenerated.{GUID}`) merge with their Start Menu
  shortcut. `"Xbox (2)"`-style duplicate suffixes are normalised.
- **Launcher helper shortcuts filtered out** — EA updaters, error reporters,
  app-recovery and uninstaller shortcuts no longer clutter the game list.
- **Steam exe detection** no longer picks console tools like `vconsole2.exe`
